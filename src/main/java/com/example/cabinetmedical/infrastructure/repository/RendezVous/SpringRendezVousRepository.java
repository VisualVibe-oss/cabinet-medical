...
public interface SpringRendezVousRepository extends JpaRepository<RendezVousEntity, Integer> {
    ...
    @Query("SELECT r FROM RendezVousEntity r " +
            "WHERE CAST(r.dateDebutRendezVous AS date) = :date " +
            "AND r.cabinet.idCabinet = :idCabinet")
    List<RendezVousEntity> findByDateAndCabinet(
            @Param("date") LocalDate date,
            @Param("idCabinet") int idCabinet);

    @Query("SELECT r FROM RendezVousEntity r WHERE r.dateDebutRendezVous BETWEEN :dateDebut AND :dateFin AND r.cabinet.idCabinet = :idCabinet")
